# 前言
针对原代码主要修改了ultralytics中的代码用于分类，包括task.py,stream_loaders.py等文件，task.py文件修改用于读取预训练模型更改最后一层以及图像输入通道，stream_loaders.py添加了用于检测手的模型，得到手的图片送入到模型中进行训练

检测手的模型基于这个https://github.com/cvlab-stonybrook/BodyHands

所有first commit是改变过部分代码，有标注
-----------------------------------------------------------------------------------------------------------------------------------------------------------------------
